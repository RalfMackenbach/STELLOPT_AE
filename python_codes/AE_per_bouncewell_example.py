...
def plot_AE_per_bouncewell(theta_arr,b_arr,dbdx_arr,lam_arr,bw,ae_list):
    c = 0.5
    # shift by pi
    fig ,ax = plt.subplots()
    ...
    cm_scale = lambda x: x
    colors_plot = [cm.plasma(cm_scale(np.asarray(x) * 0.8/max_val)) for x in ae_list]
    ...
    for idx_lam, lam in enumerate(lam_arr):
        ...
        for idx_bw, _ in enumerate(ae_list[idx_lam]):
            ...
            # if not normal plot
                ...
                ax.text(bw[idx_lam][idx_bw][1]+0.2, b_val, r'$\lambda_{{{idx}}}$'.format(idx=idx_lam),color=colors_plot[idx_lam][idx_bw])
    ...
    plt.tight_layout()
    cbar = plt.colorbar(cm.ScalarMappable(norm=mplc.Normalize(vmin=0.0, vmax=max_val, clip=False), cmap=cm.plasma), ticks=[0, max_val], ax=ax,location='bottom',label=r'$A_\lambda$') #'%.3f'
    cbar.ax.set_xticklabels([r'$0$', r'$\text{max}(A_\lambda)$'])
...
def compute_ae_over_z_gist(gist_class,omn,omt,omnigenous):
    ...
    filename = plot_AE_per_bouncewell(theta_arr,b_arr,dbdx_arr,lam_arr,bw,ae_list)
    return filename
