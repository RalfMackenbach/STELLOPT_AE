...
def compute_ae_gist(gist_class,omn,omt):
    ...
    Delta_theta = 1e-10
    del_sing = 0.0
    ae_val = ae.ae_total(q0,dlnTdx,dlnndx,Delta_x,Delta_y,b_arr,dbdx_arr,dbdy_arr,sqrt_g,theta_arr,lam_res,Delta_theta,del_sing,L_tot)
    return ae_val
...
fig = plt.figure(figsize=(5/6*6, 5/6*4.5))
ax  = fig.gca()
ax.set_yscale('log')
ax.set_xscale('log')
...
cbar = fig.colorbar(cm.ScalarMappable(norm=norm,cmap=colormap),ax=ax,ticks=[1, 2, 3, 4],label=r"density gradient")
...
plt.xlabel(r"available energy")
plt.ylabel(r"turbulent energy flux")
ax.set_axisbelow(True)
fig.tight_layout()
plt.subplots_adjust(left=0.15, right=1.0, top=0.99, bottom=0.13)
plt.savefig('/Users/ralfmackenbach/Desktop/JPP_new_figure.eps', format='eps',
